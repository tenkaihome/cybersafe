            ...
            <div className="bg-[#002b47] border border-[#00416a] rounded-md p-6 sm:p-8 shadow-sm">
                ...
                <div className="flex flex-col sm:flex-row items-end gap-6 mb-8">
                    <div className="flex-1 w-full max-w-xs">
                        <label className="block text-[15px] text-cyan-100 mb-2">Key Size</label>
                        <select
                            value={keySize}
                            onChange={(e) => setKeySize(Number(e.target.value) as 1024 | 2048 | 4096)}
                            disabled={isGenerating}
                            className="w-full bg-[#00416a] border border-transparent rounded-md px-4 py-3 text-cyan-100 focus:outline-none focus:border-[#4dbb9c] transition-colors disabled:opacity-50"
                        >
                    ...
                    <button
                        ...
                        disabled={isGenerating}
                        className="flex items-center justify-center px-6 py-3 bg-cyan-500 hover:bg-cyan-400 text-white rounded-md transition-colors text-sm font-medium shadow-sm w-full sm:w-auto h-[48px] disabled:bg-cyan-500/50 disabled:cursor-not-allowed"
                    >
                        {isGenerating ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            <label className="block text-[15px] text-cyan-100">Public Key</label>
                            {publicKey && (
                                <button
                                    onClick={() => handleCopy(publicKey, "public")}
                                    className={`flex items-center text-xs font-medium px-3 py-1 rounded transition-colors ${copiedKey === "public" ? 'text-[#4dbb9c] bg-cyan-500/10' : 'text-cyan-200/70 hover:text-cyan-200 hover:bg-[#006cae]'}`}
                                >
                        ...
                        <textarea
                            ...
                            readOnly
                            placeholder="..."
                            className="w-full bg-[#00416a] border border-transparent outline-none rounded-md px-4 py-4 text-cyan-200/70 font-mono text-[12px] min-h-[300px] resize-y"
                        />
                    </div>

                    <div>
                        <div className="flex justify-between items-center mb-2">
                            <label className="block text-[15px] text-cyan-100">Private Key</label>
                            {privateKey && (
                                <button
                                    onClick={() => handleCopy(privateKey, "private")}
                                    className={`flex items-center text-xs font-medium px-3 py-1 rounded transition-colors ${copiedKey === "private" ? 'text-[#4dbb9c] bg-cyan-500/10' : 'text-cyan-200/70 hover:text-cyan-200 hover:bg-[#006cae]'}`}
                                >
