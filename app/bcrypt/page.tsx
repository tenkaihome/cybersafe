        ...
        <div className="max-w-4xl mx-auto py-8 text-cyan-100">
            ...
            <div className="bg-[#002b47] border border-[#00416a] rounded-md p-6 sm:p-8 shadow-sm">
                <div className="grid grid-cols-1 gap-6 mb-6">
                    <div>
                        <label className="block text-[15px] text-cyan-100 mb-2">String to hash</label>
                        <input
                            type="text"
                            value={text}
                    ...
                    <div>
                        <label className="block text-[15px] text-cyan-100 mb-2">Rounds ({rounds})</label>
                        <input
                            type="range"
                            min="4"
                            max="15"
                            ...
                            className="w-full h-1 appearance-none rounded-lg cursor-pointer bg-[#006cae]"
                        />
                        <p className="text-xs text-cyan-300/50 mt-2">More rounds = more secure, but slower to generate.</p>
                    </div>

                    <div className="pt-2">
                        <button
                            onClick={generateHash}
                            className="px-6 py-2.5 bg-cyan-500 hover:bg-cyan-400 text-white rounded-md transition-colors text-sm font-medium shadow-sm w-full font-semibold"
                        >
                            Hash string
                        </button>
                    </div>
                    ...
                    {hash && (
                        <div className="mt-4">
                            <label className="block text-[15px] text-cyan-100 mb-2">Bcrypt hash result:</label>
                            <div className="flex border border-[#00416a] rounded-md overflow-hidden">
                                <input
                                    type="text"
                                    value={hash}
                                    readOnly
                                    className="flex-1 bg-[#00416a] px-4 py-3 text-cyan-100 outline-none font-mono text-[14px]"
                                />
                                <button
                                    onClick={copyToClipboard}
                                    className={`w-12 flex items-center justify-center border-l border-[#00416a] transition-colors ${isCopying ? 'text-[#4dbb9c] bg-cyan-500/10' : 'bg-[#00558c] text-cyan-200/70 hover:text-cyan-200 hover:bg-[#006cae]'}`}
                                >
