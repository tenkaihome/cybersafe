...
export default function PDFSignature() {
    ...
    return (
        <div className="max-w-4xl mx-auto py-8 text-cyan-100">
            ...
            <div className="bg-[#002b47] border border-[#00416a] rounded-md p-6 sm:p-10 shadow-sm">

                <div
                    onDragOver={(e) => { e.preventDefault(); setIsDragging(true) }}
                    onDragLeave={() => setIsDragging(false)}
                    onDrop={onDrop}
                    className={`border-2 border-dashed rounded-xl p-10 flex flex-col items-center justify-center text-center transition-all ${isDragging ? 'border-[#4dbb9c] bg-cyan-500/5' : 'border-[#00558c] hover:border-[#4dbb9c]/50 bg-[#00416a]/50'}`}
                >
                    <div className="w-16 h-16 rounded-full bg-[#00558c] flex items-center justify-center mb-4 text-cyan-200/70">
                    ...
                    <p className="text-sm text-cyan-300/50 max-w-md mx-auto mb-6">
                        Supported files: .pdf
                    </p>

                    <label className="cursor-pointer">
                        <span className="px-6 py-2.5 bg-cyan-500 hover:bg-cyan-400 text-white rounded-full transition-colors font-medium shadow-sm inline-block">
                            Select File
                        </span>
                        <input
                            type="file"
                ...
                {result && (
                    <div className="mt-8">
                        <div className={`p-4 rounded-md border ${result.includes("Error") ? "bg-red-500/10 border-red-500/20 text-red-400" : result.includes("Analyzing") ? "bg-blue-500/10 border-blue-500/20 text-blue-400" : "bg-[#00416a] border-[#00558c] text-cyan-100"}`}>
