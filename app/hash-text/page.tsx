    ...
    return (
        <div className="max-w-4xl mx-auto py-8 text-cyan-100">
            ...
            <div className="bg-[#002b47] border border-[#00416a] rounded-md p-6 sm:p-8 shadow-sm">
                <label className="block text-[15px] text-cyan-100 mb-3">Your text to hash:</label>
                <div className="relative mb-8">
                    <textarea
                        value={text}
                        onChange={(e) => setText(e.target.value)}
                        placeholder="Your string to hash..."
                        className="w-full bg-[#00416a] border border-transparent rounded-md px-4 py-4 text-cyan-100 resize-y focus:outline-none focus:border-[#4dbb9c] min-h-[120px] shadow-inner text-[15px] font-sans placeholder:text-cyan-300/50 transition-colors"
                    />
                </div>

                <div className="h-px bg-[#2e323b] w-full mb-8"></div>

                <label className="block text-[15px] text-cyan-100 mb-3">Digest encoding</label>
                <div className="relative mb-8 max-w-full">
                    <select
                        value={encoding}
                        onChange={(e) => setEncoding(e.target.value as "hex" | "base64")}
                        className="w-full bg-[#00416a] border border-transparent rounded-md px-4 py-3 text-cyan-100 appearance-none focus:outline-none focus:border-[#4dbb9c] shadow-inner text-[15px] transition-colors"
                    >
                        <option value="hex">Hexadecimal (base 16)</option>
                        <option value="base64">Base64</option>
                    </select>
                    <div className="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none text-cyan-200/70">
                        <ChevronDown className="w-4 h-4" />
                    </div>
                </div>

                <div className="flex flex-col rounded-md border border-[#00416a] overflow-hidden bg-[#00416a]">
                    {algorithms.map((algo) => {
                        const hashValue = getHash(algo.method)
                        return (
                            <div
                                key={algo.name}
                                className={`flex items-stretch border-b border-[#00416a] last:border-b-0`}
                            >
                                <div className="w-[120px] sm:w-[150px] px-4 py-3 text-[14px] font-medium text-white bg-[#00558c] border-r border-[#00416a] shrink-0 flex items-center">
                                    {algo.name}
                                </div>
                                <div className="flex-1 px-4 py-3 text-[14px] text-cyan-100 font-mono break-all flex items-center min-h-[46px] selection:bg-cyan-500/30">
                                    {hashValue}
                                </div>
                                <button
                                    onClick={() => handleCopy(hashValue, algo.name)}
                                    disabled={!text}
                                    className={`w-[46px] flex-shrink-0 flex items-center justify-center border-l border-[#00416a] transition-colors ${!text ? 'text-zinc-600 cursor-not-allowed' : copiedHash === algo.name ? 'text-[#4dbb9c] bg-cyan-500/10' : 'text-cyan-200/70 hover:text-cyan-200 hover:bg-[#006cae]'}`}
